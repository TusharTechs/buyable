...
export { AnthropicProvider, BedrockProvider };

export interface ProviderOptions {
  region: string;
  /** "bedrock" | "anthropic". Defaults to the BUYABLE_PROVIDER environment variable. */
  provider?: string;
...
export function createProvider(opts: ProviderOptions): ReasoningProvider {
  ...
  if (!key) {
    throw new Error(
      "No reasoning provider available. Set ANTHROPIC_API_KEY, or set BUYABLE_PROVIDER=bedrock on an account whose Bedrock access is not restricted.",
    );
  }
